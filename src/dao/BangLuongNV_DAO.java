...
public class BangLuongNV_DAO {
	...
	public boolean kiemTraMaLuongNV(String maLuongNV){
		ConnectDB.getInstance();
		Connection con = ConnectDB.getConnection();
		String sql = "select * from BangLuongNhanVien where maLuongNV = ?";
		PreparedStatement stmt = null;
	...
	public void taoBangLuongNV(NhanVien nv,ArrayList<BangChamCongNV> listccnv){
		BangLuongNV blnv = new BangLuongNV();
		blnv.setMaLuongNV(nv.getMaNV()+listccnv.get(0).getNgayCham().getMonthValue()+listccnv.get(0).getNgayCham().getYear());
		blnv.setNv(nv);
		blnv.setThang(listccnv.get(0).getNgayCham().getMonthValue());
		blnv.setNam(listccnv.get(0).getNgayCham().getYear());
		blnv.setSoNgayDiLam(0);
		blnv.setSoNgayNghiKhongPhep(0);
		blnv.setTienPhat(0);
		for(BangChamCongNV ccnv : listccnv){
			if(ccnv.isCoMat() == true){
				blnv.setSoNgayDiLam(blnv.getSoNgayDiLam()+1);
			}
			else{
				blnv.setSoNgayNghiKhongPhep(blnv.getSoNgayNghiKhongPhep()+1);
			}
			if(ccnv.isVangMat() == true){
				blnv.setTienPhat(blnv.getTienPhat()+100000);
			}
			if(ccnv.isCoPhep() == true){
				blnv.setTienPhat(blnv.getTienPhat()+50000);
			}
		}
		insertBangLuongNV(blnv);
	}
	public void capNhatBangLuongNVTong(NhanVien nv,ArrayList<BangChamCongNV> listccnv){
		BangLuongNV blnv = getMotBangLuongNV(nv.getMaNV()+listccnv.get(0).getNgayCham().getMonthValue()+listccnv.get(0).getNgayCham().getYear());
		blnv.setSoNgayDiLam(0);
		blnv.setSoNgayNghiKhongPhep(0);
		blnv.setTienPhat(0);
		for(BangChamCongNV ccnv : listccnv){
